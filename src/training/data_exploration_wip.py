...
## 'offSubmissionSuccessfulShare',
# 'offSubmissionAttemptedShare',
# 'offKnockdownsShare',
# 'offPassSuccessfulShare',
# 'offReversalSuccessfulShare']]
data = data[['offBodySigStrikeAccuracy',
 'offBodySigStrikeAttempted',
 'offBodySigStrikeSuccessful',
 'offClinchSigStrikeAccuracy',
 'offClinchSigStrikeAttempted',
...
columns = ['offBodySigStrikeAccuracy',
 'offBodySigStrikeAttempted',
 'offBodySigStrikeSuccessful',
 'offClinchSigStrikeAccuracy',
 'offClinchSigStrikeAttempted',
...
for solver in solvers:
    for weight in weights:
        ...
        reg = LogisticRegressionCV(cv=5, random_state=0, class_weight=weight, solver = solver)
        #reg = LogisticRegression(class_weight=None, solver = 'saga')
        # The "accuracy" scoring is proportional to the number of correct
        # classifications
        rfecv = RFECV(estimator=reg, step=1, cv=StratifiedKFold(6),
                      scoring='neg_log_loss')
        rfecv.fit(X, y)
        print("Optimal number of features : %d (%.4f)" % (rfecv.n_features_, rfecv.grid_scores_[rfecv.n_features_-1]))
        scores[n] = {'solver': solver, 'weight': weight, 'score': rfecv.grid_scores_[rfecv.n_features_-1]}
...
for solver in solvers:
    for weight in weights:
        ...
        rfecv.fit(X, y)
        print("Optimal number of features : %d (%.4f)" % (rfecv.n_features_, rfecv.grid_scores_[rfecv.n_features_-1]))
        roc_scores[n] = {'solver': solver, 'weight': weight, 'score': rfecv.grid_scores_[rfecv.n_features_-1]}
        # Plot number of features VS. cross-validation scores
        plt.figure()
        plt.xlabel("Number of features selected")
        plt.ylabel("Cross validation score (nb of correct classifications)")
...
{1: {'solver': 'newton-cg', 'weight': 'balanced', 'score': 0.9681854669385586},
...
for solver in solvers:
    for weight in weights:
        ...
        rfecv.fit(X, y)
        print("Optimal number of features : %d (%.4f)" % (rfecv.n_features_, rfecv.grid_scores_[rfecv.n_features_-1]))
        f1_scores[n] = {'solver': solver, 'weight': weight, 'score': rfecv.grid_scores_[rfecv.n_features_-1]}
        # Plot number of features VS. cross-validation scores
        plt.figure()
        plt.xlabel("Number of features selected")
        plt.ylabel("Cross validation score (nb of correct classifications)")
...
{1: {'solver': 'newton-cg', 'weight': 'balanced', 'score': 0.6790100509690579},
...
cols = ['offKnockdowns', 'offGroundSigStrikeSuccessful', 'offBodySigStrikeSuccessful', 
        'offHeadSigStrikeAttempted', 'offHeadSigStrikeSuccessful']
